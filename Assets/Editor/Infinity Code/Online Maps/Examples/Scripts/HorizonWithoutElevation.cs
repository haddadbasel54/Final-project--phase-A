...
namespace OnlineMapsDemos
{
    ...
    public class HorizonWithoutElevation : MonoBehaviour
    {
        ...
        private void InitMesh()
        {
            ...
            extraTiles = new List<Tile>(countX * 2 + countY * 2 + 4);
        ...
        private void PreloadBorders()
        {
            ...
            int itx = Mathf.RoundToInt((float) (t.x - countX / 2f));
            int ity = Mathf.RoundToInt((float) (t.y - countY / 2f));

            foreach (Tile tile in extraTiles) tile.Unblock(this);
            extraTiles.Clear();
            ...
            for (int x = -1; x < countX + 1; x++)
            {
                ...
                Tile tile = tileManager.GetTile(zoom, tileX, tileY);
                if (tile == null)
                {
                    Tile parentTile = tileManager.GetTile(zoom - 1, tileX / 2, tileY / 2);
                    tile = new RasterTile(tileX, tileY, zoom, map);
        ...
        private void Start()
        {
            ...
            if (!shader) shader = RenderPipelineHelper.GetDefaultShader();
            ...
            if (Cache.instance)
            {
                Cache.instance.OnLoadedFromCache += OnTileDownloaded;
            }
        ...
        private void UpdateMesh()
        {
            ...
            int zoom = view.intZoom - zoomOffset;
            if (zoom < 3) zoom = 3;
            if (zoom >= view.intZoom)
            {
            ...
            TilePoint t = view.GetCenterTile(zoom);
            ...
            int zOffset = view.intZoom - zoom;
            Vector2 sizeInScene = control.sizeInScene;
            Vector2 tileSize = new Vector2( 
                sizeInScene.x / view.countTilesX * (1 << zOffset) / view.zoomFactor,
                sizeInScene.y / view.countTilesY * (1 << zOffset) / view.zoomFactor
                );

            GeoPoint p = view.projection.TileToLocation(itx + countX / 2f, ity + countY / 2f, zoom);

            Vector3 startWorldPos = new Vector3(0, positionYOffset, 0)
                                    + Quaternion.Inverse(map.transform.rotation) * p.ToWorld(control) 
            ...
            for (int x = 0; x < countX; x++)
            {
                ...
                for (int y = 0; y < countY; y++)
                {
                    ...
                    Tile tile = map.tileManager.GetTile(zoom, tileX, tileY);
                    if (tile == null)
                    {
                        Tile parentTile = map.tileManager.GetTile(zoom - 1, tileX / 2, tileY / 2);
                        tile = new RasterTile(tileX, tileY, zoom, map);
                        tile.parent = parentTile;
                    }
                    ...
                    for (int vx = 0; vx < resolution; vx++)
                    {
                        ...
                        {
                            Vector3 v = new Vector3(ov.x * vx + v1.x, positionYOffset, ov.z * vz + v1.z);
