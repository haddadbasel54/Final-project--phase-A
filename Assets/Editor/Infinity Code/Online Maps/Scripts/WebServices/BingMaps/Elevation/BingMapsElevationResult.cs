...
namespace OnlineMaps.Webservices
{
    ...
    /// </summary>
    public class BingMapsElevationResult
    {
        /// <summary>
        ...
        /// Constructor
        /// </summary>
        public BingMapsElevationResult(){}

        /// <summary>
        /// Constructor
        /// </summary>
        /// <param name="xml">Response XML</param>
        public BingMapsElevationResult(XML xml)
        {
            ...
            foreach (XML node in xml)
            {
                ...
                else if (node.name == "ResourceSets") foreach (XML rsNode in node) rs.Add(new ResourceSet(rsNode));
            }
        ...
        public class ResourceSet
        {
            ...
            public ResourceSet(XML xml)
            {
                ...
                foreach (XML node in xml)
                {
                    ...
                    else if (node.name == "Resources") foreach (XML rsNode in node) rs.Add(new Resource(rsNode));
                }
        ...
        public class Resource
        {
            ...
            public Resource(XML xml)
            {
                ...
                foreach (XML node in xml[0])
                {
                    if (node.name == "ZoomLevel") zoomLevel = node.Value<int>();
                    else if (node.name == "Elevations") foreach (XML rsNode in node) el.Add(rsNode.Value<int>());
                    else if (node.name == "Offsets") foreach (XML rsNode in node) el.Add(rsNode.Value<int>());
                }
        ...
        public static BingMapsElevationResult Parse(string response)
        {
            try
            {
                return JSON.Deserialize<BingMapsElevationResult>(response);
            }
            catch
            {
            }
