...
namespace OnlineMaps
{
    public partial class Cache
    {
        ...
        /// </summary>
        public Action<Tile> OnLoadedFromFileCache;
        ...
        private FileAtlas fileCacheAtlas;
        ...
        private void AddFileCacheItem(Tile tile, byte[] bytes)
        {
        ...
        public StringBuilder GetFileCacheFolder()
        {
            ...
            {
                stringBuilder.Append(Application.persistentDataPath).Append("/").Append("OnlineMapsCache");
            }
            else
            {
                if (!string.IsNullOrEmpty(fileCacheCustomPath))
                {
        ...
        public int GetFileCacheSizeFast()
        {
            ...
            builder.Append("/").Append(FileAtlas.AtlasName);
        ...
        public StringBuilder GetShortTilePath(Tile tile)
        {
            RasterTile rTile = tile as RasterTile;
            MapType mapType = rTile != null ? rTile.mapType : tile.map.activeType;
            return GetShortTilePath(tile, mapType);
        }

        /// <summary>
        /// Get the relative path to the tile in the file cache with the specified map type.
        /// </summary>
        /// <param name="tile">Tile</param>
        /// <param name="mapType">Map Type</param>
        /// <returns>Relative path to the tile in the file cache</returns>
        public StringBuilder GetShortTilePath(Tile tile, MapType mapType)
        {
#if ALLOW_FILECACHE
            int startIndex = 0;
            StringBuilder stringBuilder = GetStringBuilder();
                ...
                if (c == '{')
                {
                    for (int j = i + 1; j < l; j++)
                    {
                        ...
                        if (c == '}')
                        {
                            ...
                            else if (v == "quad") Utils.TileToQuadKey(tile.x, tile.y, tile.zoom, stringBuilder);
        ...
        public bool GetTileTexture(Tile tile, MapType mapType, out Texture2D texture)
        {
            texture = null;
#if ALLOW_FILECACHE
            if (fileCacheAtlas == null) LoadFileCacheAtlas();
        ...
        private void LoadTile(Tile tile, byte[] bytes)
        {
            ...
            {
                (tile as RasterTile).ApplyTexture(texture);
            ...
            else
            {
                ...
                TileSetControl tsControl = map.control as TileSetControl;
                if (tsControl && tsControl.compressTextures) texture.Compress(true);
            }
        ...
        /// <summary>
        /// Sets the tile texture in the file cache.
        /// </summary>
        /// <param name="tile">Tile</param>
        /// <param name="mapType">Map Type</param>
        /// <param name="texture">Texture</param>
        public void SetTileTexture(Tile tile, MapType mapType, Texture2D texture)
        {
#if ALLOW_FILECACHE
            if (fileCacheAtlas == null) LoadFileCacheAtlas();
        ...
        public bool TryLoadFromCache(Tile tile)
        {
#if ALLOW_FILECACHE
        ...
        private bool TryLoadFromFileCache(Tile tile)
        {
#if ALLOW_FILECACHE
            if (fileCacheAtlas == null) LoadFileCacheAtlas();
            ...
            ThreadManager.AddThreadAction(() =>
            {
                ...
                ThreadManager.AddMainThreadAction(() =>
                {
                    ...
                    Log.Info("Tile " + tile + " loaded from cache.", Log.Type.cache);
                });
            });
#else
            byte[] bytes = File.ReadAllBytes(fullTilePath);
        ...
        /// File cache atlas
        /// </summary>
        public class FileAtlas : CacheAtlas<FileItem>
        {
            /// <summary>
            ...
            /// </summary>
            protected override string atlasName => AtlasName;
            ...
            public void Add(Cache cache, Tile tile, byte[] bytes)
            {
                RasterTile rTile = tile as RasterTile;
                MapType mapType = rTile != null ? rTile.mapType : tile.map.activeType;
            ...
            /// <param name="mapType">Map Type</param>
            /// <param name="bytes">Bytes of tile</param>
            public void Add(Cache cache, Tile tile, MapType mapType, byte[] bytes)
            {
#if ALLOW_FILECACHE
            ...
            {
                FileItem item = new FileItem(filename, size);
                if (capacity <= count)
                {
                    capacity += 100;
            ...
            public override FileItem CreateItem(string filename, int size, long time)
            {
                return new FileItem(filename, size, time);
            }

            public override void DeleteOldItems(Cache cache)
            {
#if ALLOW_FILECACHE
            ...
            public string GetFullPath(Cache cache, string shortFilename)
            {
#if ALLOW_FILECACHE
        ...
        /// File cache item
        /// </summary>
        public class FileItem : CacheItem
        {
            /// <summary>
            /// Constructor
            /// </summary>
            ...
            public FileItem(string filename, int size) : this(filename, size, DateTime.Now.Ticks)
            {
            }
            ...
            public FileItem(string filename, int size, long time)
            {
                key = filename;
