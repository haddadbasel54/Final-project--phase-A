...
using System.Linq;
using System.Text;
using LPoints = System.Collections.Generic.List<OnlineMaps.VectorTile.LPoint>;
...
namespace OnlineMaps
{
    ...
    /// </summary>
    public abstract class VectorTile : Tile
    {
        ...
        public VectorTile(int x, int y, int zoom, Map map, bool isMapTile = true) : base(x, y, zoom, map, isMapTile)
        {
        }
        ...
        private static List<LPoints> ClipGeometries(List<LPoints> geoms, GeomType geomType, long extent, uint bufferSize)
        {
            ...
            MapboxClipper.Clipper c = new MapboxClipper.Clipper();
            c.AddPaths(subjects, MapboxClipper.PolyType.ptSubject, closed);
            c.AddPaths(clip, MapboxClipper.PolyType.ptClip, true);
            ...
            {
                MapboxClipper.PolyTree lineSolution = new MapboxClipper.PolyTree();
                succeeded = c.Execute(MapboxClipper.ClipType.ctIntersection, lineSolution, MapboxClipper.PolyFillType.pftNonZero, MapboxClipper.PolyFillType.pftNonZero);
                if (succeeded) solution = MapboxClipper.Clipper.PolyTreeToPaths(lineSolution);
            }
            else
            {
                succeeded = c.Execute(MapboxClipper.ClipType.ctIntersection, solution, MapboxClipper.PolyFillType.pftNonZero, MapboxClipper.PolyFillType.pftNonZero);
            }
        ...
        public class Layer
        {
            ...
            {
                return VectorTile.GetFeature(this, index);
            }
        }
