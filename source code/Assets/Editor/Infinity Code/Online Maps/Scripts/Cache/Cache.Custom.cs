...
namespace OnlineMaps
{
    public partial class Cache
    {
        ...
        private CustomAtlas customCacheAtlas;
        ...
        {
            if (_instance) _instance.AddItem(key, bytes);
        }
        ...
        public void ClearCustomCache()
        {
            ...
            customCacheAtlas = new CustomAtlas();
        ...
        {
            return _instance ? _instance.GetItem(key) : null;
        }
        ...
        public int GetCustomCacheSizeFast()
        {
            ...
            builder.Append("/").Append(CustomAtlas.AtlasName);
        ...
        /// Custom cache atlas.
        /// </summary>
        public class CustomAtlas : CacheAtlas<CustomItem>
        {
            /// <summary>
            ...
            public void Add(Cache cache, string key, byte[] bytes)
            {
#if ALLOW_FILECACHE
                ...
                if (index == -1)
                {
                    CustomItem item = AddItem(key, bytes.Length);
                    path = item.GetFullPath(cache);
                    size += bytes.Length;
                }
            ...
            private CustomItem AddItem(string key, int size)
            {
                CustomItem item = new CustomItem(key, size);
                if (capacity <= count)
                {
                    capacity += 100;
            ...
            public override CustomItem CreateItem(string filename, int size, long time)
            {
                return new CustomItem(filename, size, time);
            }
            ...
            public byte[] GetItem(Cache cache, string key)
            {
                int hash = key.GetHashCode();
        ...
        /// Custom cache item.
        /// </summary>
        public class CustomItem : CacheItem
        {
            /// <summary>
            /// Constructor.
            /// </summary>
            ...
            public CustomItem(string key, int size) : this(key, size, DateTime.Now.Ticks)
            {
            }
            ...
            public CustomItem(string key, int size, long time)
            {
                this.key = key;
            ...
            /// <param name="cache">Cache</param>
            /// <returns>Value or null</returns>
            public byte[] GetBytes(Cache cache)
            {
#if ALLOW_FILECACHE
            ...
            public string GetFullPath(Cache cache)
            {
#if ALLOW_FILECACHE
