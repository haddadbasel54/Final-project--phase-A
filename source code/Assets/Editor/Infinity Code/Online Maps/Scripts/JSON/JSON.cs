...
namespace OnlineMaps
{
    ...
    public class JSON
    {
        ...
        private static object DeserializeValue(Type type, object obj)
        {
            ...
            try
            {
                return Convert.ChangeType(obj, type, Culture.numberFormat);
            }
        ...
        public static JSONItem Serialize(object obj, SerializeSettings settings = SerializeSettings.Default)
        {
            if (obj is Action) return new JSONValue(obj, JSONValueType.NULL);
#if !UNITY_WP_8_1 || UNITY_EDITOR
            if (obj == null || obj is DBNull) return new JSONValue(obj, JSONValueType.NULL);
#else
            if (obj == null) return new JSONValue(obj, ValueType.NULL);
#endif
            ...
            if (obj is IDictionary)
            {
                ...
                while (keysEnum.MoveNext() && valuesEnum.MoveNext())
                {
                    ...
                    dv.Add(k as string, Serialize(v, settings & ~SerializeSettings.SerializeUnityObject));
                }
            ...
            if (obj is IEnumerable)
            {
                ...
                JSONArray array = new JSONArray();
                foreach (var item in v) array.Add(Serialize(item, settings & ~SerializeSettings.SerializeUnityObject));
                return array;
            }
            ...
            if (settings.HasFlag(SerializeSettings.NonPublic) || ReflectionHelper.CheckIfAnonymousType(type))
            {
                bindingFlags |= BindingFlags.NonPublic;
            }

            IEnumerable<FieldInfo> fields = ReflectionHelper.GetFields(type, bindingFlags);
            foreach (FieldInfo field in fields)
            {
                ...
                if (field.GetCustomAttribute<JsonNonSerializedAttribute>() != null || field.GetCustomAttribute<NonSerializedAttribute>() != null)
                {
                    continue;
                }

                o.Add(fieldName, Serialize(field.GetValue(obj), settings & ~SerializeSettings.SerializeUnityObject));
            }
        ...
        /// </summary>
        [Flags]
        public enum SerializeSettings
        {
            /// <summary>
            /// Default serialization settings.
            /// </summary>
            Default = SerializeUnityObject | NonPublic,

            /// <summary>
            /// Serialize Unity objects.
            /// </summary>
            SerializeUnityObject = 1 << 0,

            /// <summary>
            /// Include non-public members in serialization.
            /// </summary>
            NonPublic = 1 << 1,
        }
    }
