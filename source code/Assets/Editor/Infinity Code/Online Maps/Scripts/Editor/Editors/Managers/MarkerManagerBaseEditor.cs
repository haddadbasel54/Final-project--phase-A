...
{
    public abstract class MarkerManagerBaseEditor<T, U> : Editor 
        ...
        where U: Marker
    {
        ...
        protected virtual void DrawItem(int i, ref int removedIndex)
        {
            ...
            MarkerBasePropertyDrawer.isRemoved = false;
            MarkerBasePropertyDrawer.isEnabledChanged = null;
            EditorGUILayout.PropertyField(items.GetArrayElementAtIndex(i), TempContent.Get("Marker " + (i + 1)));
            if (MarkerBasePropertyDrawer.isRemoved) removedIndex = i;
            if (MarkerBasePropertyDrawer.isEnabledChanged.HasValue) manager[i].enabled = MarkerBasePropertyDrawer.isEnabledChanged.Value;
        ...
        private void DrawItemHeader(bool usePagination, int start, int end, int countPages)
        {
            ...
            else
            {
                ...
                GUIContent pageLabel = TempContent.Get(page + "/" + countPages);
