...
namespace OnlineMaps.Webservices
{
    ...
    public class GooglePlaceDetailsResult
    {
        ...
        /// </summary>
        public GooglePlacesResult.Photo[] photos;
        ...
        /// Default constructor.
        /// </summary>
        public GooglePlaceDetailsResult()
        {
        }

        /// <summary>
        /// Constructor of GooglePlaceDetailsResult.
        /// </summary>
        /// <param name="node">Place node from response.</param>
        public GooglePlaceDetailsResult(XML node)
        {
            this.node = node;
            formatted_address = node.Get("formatted_address");
            formatted_phone_number = node.Get("formatted_phone_number");

            XML locationNode = node.Find("geometry/location");
            ...
            XMLList photosList = node.FindAll("photo");
            photos = new GooglePlacesResult.Photo[photosList.count];
            for (int i = 0; i < photosList.count; i++) photos[i] = new GooglePlacesResult.Photo(photosList[i]);
            ...
            XMLList typeNode = node.FindAll("type");
        ...
        public static GooglePlaceDetailsResult Parse(string response)
        {
            try
            {
                ...
                return new GooglePlaceDetailsResult(xml["result"]);
            }
            catch
            {
            }
