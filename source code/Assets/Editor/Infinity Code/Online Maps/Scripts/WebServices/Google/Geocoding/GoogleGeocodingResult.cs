...
namespace OnlineMaps.Webservices
{
    ...
    public class GoogleGeocodingResult
    {
        ...
        /// </summary>
        public GeoPoint geometry_location;
        ...
        /// </summary>
        public GeoPoint geometry_viewport_northeast;
        ...
        /// Generally the viewport is used to frame a result when displaying it to a user.
        /// </summary>
        public GeoPoint geometry_viewport_southwest;
        ...
        /// </summary>
        public GeoPoint geometry_bounds_northeast;
        ...
        /// Note that these bounds may not match the recommended viewport.
        /// </summary>
        public GeoPoint geometry_bounds_southwest;
        ...
        /// <summary>
        /// Initializes a new instance of the GoogleGeocodingResult class.
        /// </summary>
        ...
        /// <summary>
        /// Constructor of GoogleGeocodingResult.
        /// </summary>
        /// <param name="node">Location node from response</param>
        public GoogleGeocodingResult(XML node)
        {
            ...
            foreach (XML n in node)
            {
                ...
                else if (n.name == "geometry")
                {
                    foreach (XML gn in n)
                    {
                        if (gn.name == "location") geometry_location = XML.GetVector2FromNode(gn);
                        else if (gn.name == "location_type") geometry_location_type = gn.Value();
                        else if (gn.name == "viewport")
                        {
                            geometry_viewport_northeast = XML.GetVector2FromNode(gn["northeast"]);
                            geometry_viewport_southwest = XML.GetVector2FromNode(gn["southwest"]);
                        }
                        else if (gn.name == "bounds")
                        {
                            geometry_bounds_northeast = XML.GetVector2FromNode(gn["northeast"]);
                            geometry_bounds_southwest = XML.GetVector2FromNode(gn["southwest"]);
                        }
        ...
        public static GoogleGeocodingResult[] Parse(string response)
        {
            try
            {
                ...
                List<GoogleGeocodingResult> results = new List<GoogleGeocodingResult>();

                XMLList resNodes = xml.FindAll("//result");

                foreach (XML node in resNodes)
                {
                    results.Add(new GoogleGeocodingResult(node));
                }
        ...
        public class AddressComponent
        {
            ...
            /// <summary>
            /// Initializes a new instance of the AddressComponent class.
            /// </summary>
            public AddressComponent()
            {
            }

            public AddressComponent(XML node)
            {
                List<string> types = new List<string>();
            ...
            {
                return "GoogleGeocodingResult.AddressComponent. Types: {" + string.Join(",", types) + "}, Long name: {" + long_name + "}, Short name: {" + short_name + "}";
            }
        }
    }
}